...
# USAGE: uv run get_openslr_rir_data.py --data_root=<where to put data>
...
def __process_data(data_folder: str, dst_folder: str, manifest_file: str):
    ...
    for rir_f in tqdm(rirfiles, desc="Processing RIR files"):
        info = sf.info(rir_f)
        n_chans = info.channels
        if n_chans == 1:
            copy(rir_f, real_rir_folder)
        else:
            # Read the entire multi-channel audio file
            data, samplerate = sf.read(rir_f)
            for chan in range(n_chans):
                ...
                # data[:, chan] extracts the channel from the multi-channel array
                sf.write(chan_file_name, data[:, chan], samplerate)
    ...
    with open(manifest_file, "w") as man_f:
        entry = {}
        for rir in tqdm(all_rirs, desc="Creating manifest"):
            rir_file = os.path.join(dst_folder, rir)
            info = sf.info(rir_file)
...
if __name__ == "__main__":
    main()
# source: https://huggingface.co/camenduru/NeMo/blob/main/scripts/dataset_processing/get_openslr_rir_data.py
